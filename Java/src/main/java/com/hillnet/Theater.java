 ...
 * city. He knows a lot about art and his advice is usually good, but not this time: 
...
the view (even if only for a couple of seconds) of all the people who sit behind you and in your column or the columns to your left.
 To gain some courage, you decide to calculate the number of such people and see if you can possibly make it to the exit without disturbing 
 too many people.
...
public class Theater {
	public int seatsInATheater(int nCols, int nRows, int col, int row) {
		int xrows = nRows - (row);
		int xcols = nCols - col + 1;
		int people = xrows * xcols;
		return people;
	}
}
